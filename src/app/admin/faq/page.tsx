    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white rounded-xl border p-6 shadow-sm mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900">{editing ? "Edit" : "Add"} FAQ</h2>
            <button type="button" onClick={() => setShowForm(false)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
          </div>
          <div className="mb-4">
            <input required placeholder="Question" value={form.question || ""} onChange={(e) => setForm({ ...form, question: e.target.value })} className="w-full rounded-lg border px-3 py-2 text-sm mb-3" />
            <textarea required placeholder="Answer" value={form.answer || ""} onChange={(e) => setForm({ ...form, answer: e.target.value })} className="w-full rounded-lg border px-3 py-2 text-sm mb-3" rows={3} />
            <input required type="number" placeholder="Order" value={form.order || 0} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} className="w-full rounded-lg border px-3 py-2 text-sm" />
          </div>
      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50"><tr><th className="text-left font-medium text-gray-500 px-5 py-3">Question</th><th className="text-left font-medium text-gray-500 px-5 py-3">Order</th><th className="text-right font-medium text-gray-500 px-5 py-3">Actions</th></tr></thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-t">
                <td className="px-5 py-3 font-medium text-gray-900">{item.question}</td>
            ...
            {items.length === 0 && <tr><td colSpan={3} className="px-5 py-8 text-center text-gray-400">No FAQs yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
