...
export default function CertificatesPage() {
  ...
  const [form, setForm] = useState<Partial<Certificate>>({ order: 0 });
  ...
  async function save(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      await supabase.from("certificates").update(form).eq("id", editing.id);
    } else {
  ...
  async function handleConfirmDelete() {
    ...
    await supabase.from("certificates").delete().eq("id", selectedId);
    setDialogOpen(false);
    setSelectedId(null);
    load();
  }
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white rounded-xl border p-6 shadow-sm mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900">{editing ? "Edit" : "Add"} Certificate</h2>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            ...
            <input required placeholder="Type (ISO/Approval/Registration)" value={form.type || ""} onChange={(e) => setForm({ ...form, type: e.target.value })} className="rounded-lg border px-3 py-2 text-sm" />
      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {items.length === 0 && <tr><td colSpan={4} className="px-5 py-8 text-center text-gray-400">No certificates yet</td></tr>}
          </tbody>
        </table>
      </div>
      <ConfirmDialog
