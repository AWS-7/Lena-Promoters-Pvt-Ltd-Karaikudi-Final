...
export default function ServicesPage() {
  ...
  const [form, setForm] = useState<Partial<Service>>({ order: 0 });
  ...
  async function handleConfirmDelete() {
    ...
    await supabase.from("services").delete().eq("id", selectedId);
    setDialogOpen(false);
    setSelectedId(null);
    load();
  }
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white rounded-xl border p-6 shadow-sm mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900">{editing ? "Edit" : "Add"} Service</h2>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            ...
            <input required placeholder="Icon name" value={form.icon || ""} onChange={(e) => setForm({ ...form, icon: e.target.value })} className="rounded-lg border px-3 py-2 text-sm" />
          ...
          <textarea required placeholder="Description" value={form.description || ""} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full rounded-lg border px-3 py-2 text-sm mb-4" rows={3} />
      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50"><tr><th className="text-left font-medium text-gray-500 px-5 py-3">Title</th><th className="text-left font-medium text-gray-500 px-5 py-3">Order</th><th className="text-right font-medium text-gray-500 px-5 py-3">Actions</th></tr></thead>
          <tbody>
            {items.map((item) => (
            ...
            {items.length === 0 && <tr><td colSpan={3} className="px-5 py-8 text-center text-gray-400">No services yet</td></tr>}
          </tbody>
        </table>
      </div>
      <ConfirmDialog
