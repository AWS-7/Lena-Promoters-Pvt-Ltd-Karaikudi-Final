...
export default function PartnersPage() {
  ...
  const [form, setForm] = useState<Partial<Partner>>({ order: 0 });
  ...
  async function handleConfirmDelete() {
    ...
    await supabase.from("partners").delete().eq("id", selectedId);
    setDialogOpen(false);
    setSelectedId(null);
    load();
  }
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white rounded-xl border p-6 shadow-sm mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            ...
            <input required placeholder="Type (Bank/Finance)" value={form.type || ""} onChange={(e) => setForm({ ...form, type: e.target.value })} className="rounded-lg border px-3 py-2 text-sm" />
            <input placeholder="Logo URL" value={form.logo_url || ""} onChange={(e) => setForm({ ...form, logo_url: e.target.value })} className="rounded-lg border px-3 py-2 text-sm" />
            <input placeholder="Website" value={form.website || ""} onChange={(e) => setForm({ ...form, website: e.target.value })} className="rounded-lg border px-3 py-2 text-sm" />
      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {items.length === 0 && <tr><td colSpan={4} className="px-5 py-8 text-center text-gray-400">No partners yet</td></tr>}
          </tbody>
        </table>
      </div>
      <ConfirmDialog
