...
export function SiteVisitFormBox({ embedded = false }: { embedded?: boolean }) {
  return (
    <Suspense fallback={<div className={embedded ? "min-h-[420px]" : "py-16 md:py-24 bg-gray-50"} />}>
      <SiteVisitFormContent embedded={embedded} />
    </Suspense>
  );
}

function SiteVisitFormContent({ embedded }: { embedded: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await supabase.from("site_visit_bookings").insert([{ ...form, status: "pending" }]);
          ...
          {!embedded && (
            <>
              ...
              <p className="text-gray-500 text-sm mb-6">
                Schedule a complimentary site visit to any of our layouts.
              </p>
            </>
          )}
  ...
  if (embedded) {
    return <div id="site-visit">{formCard}</div>;
  }
